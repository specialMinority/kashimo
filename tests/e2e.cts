...
(async () => {
    ...
    try {
        ...
        results.push('Filtered selection deletes only selected IDs, preserving hidden and deselected records');
        await page.reload();
        await page.getByText('いまの貸し借り',{exact:true}).waitFor();
        assert.equal(await page.getByText('¥0',{exact:true}).count(),1);
        assert.equal(await page.getByText('¥3,800',{exact:true}).count()>=1,true);
        results.push('Reload preserves deletion and dashboard totals update');
        ...
        assert.equal((await records()).length,0);
        results.push('Delete-all empty state persists after restart');
        ...
        results.push('Storage error preserves data and selection; retry succeeds');

        await seed([]);
        await tab('追加');
        await page.getByLabel('相手の名前',{exact:true}).fill('テスト友だち');
        await page.getByLabel('金額',{exact:true}).fill('2500');
        await page.getByLabel('メモ',{exact:true}).fill('端末内のテスト');
        await page.screenshot({path:'docs/add-mobile.png'});
        ...
        results.push('Existing create/edit flow works and returning detail refreshes immediately');
        ...
        results.push('PWA manifest, local icons and offline cold reload retain records');
        ...
        for(const width of [320,768,1440]) {
            ...
            await p.screenshot({path:'docs/home-'+width+'.png'});
            await p.getByText('設定',{exact:true}).last().click();
            await p.getByText('あなたらしく、使おう。',{exact:true}).waitFor();
            assert.equal(await p.evaluate(()=>document.documentElement.scrollWidth <= innerWidth),true);
            await c.close();
        }
        results.push('320/390/768/1440 pixel layouts and settings render without horizontal overflow');
        assert.deepEqual(errors,[]);
        results.push('No uncaught browser runtime errors');
